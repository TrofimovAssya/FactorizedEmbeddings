...
class DomainGeneDataset(Dataset):
    ...
    def __init__(self,root_dir='.',save_dir='.',data_file='data.npy', domain_file = 'domain.npy', transform=True):
        ...
        self.X_data, self.Y_data = self.dataset_make(self.data,log_transform=self.transform, self.domain)
    ...
    def dataset_make(self, gene_exp, log_transform=False, domains):
        indices_p1 = np.arange(gene_exp.shape[0])
        indices_g = np.arange(gene_exp.shape[1])
...
def get_dataset(opt,exp_dir):
    ...
    if opt.dataset == 'gene':
        ...
        dataset = DomainGeneDataset(root_dir=opt.data_dir, save_dir = exp_dir, data_file = opt.data_file, domains = opt.data_domain, transform = opt.transform)
    else:
