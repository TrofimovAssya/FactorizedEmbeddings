...
class GeneDataset(Dataset):
    ...
    def __init__(self, root_dir='.',save_dir='.',data_path='small_gtex_data.npy',data_type_path='small_gtex_types.npy',data_subtype='small_gtex_subtypes.npy', transform=None):
...
class KmerDataset(Dataset):
    ...
    def __init__(self, root_dir='/data/milatmp1/dutilfra/dataset/kmer/', data_path='duodenum1.24.hdf5', tmp_folder='/Tmp/dutilfra/dataset/kmer/' , transform=None):
        ...
        if not os.path.exists(tmp_folder):
            print( f"Copying the data in {tmp_path}...")
...
def get_dataset(opt,exp_dir):
    ...
    if opt.dataset == 'gene':
        dataset = GeneDataset(root_dir=opt.data_dir, save_dir = exp_dir)
